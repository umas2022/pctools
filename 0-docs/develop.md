...
- 指定关键字删除时指定关键字出现的位置（头、尾）
- 爬虫程序运行的非常慢,不就是个截图怎么这么慢啊,并行还报错
- 爬虫对loading页的判断不太好,2k+png->36k,4k+jpg->169k,仅凭图片尺寸判断loading需要根据截图环境调整阈值
- 7zip压缩程序运行速度非常慢,而且没有进度条,大批量处理时后面会卡死,看任务管理器没有资源占用,怀疑是被windows杀死了
- 使用os.path.normpath()替代replace("\\", "/")
- 没填输出路径时直接使用输入路径
- 关于输入参数检查:加一个utils,顺便把每次的输入写进intf.json里,保存上次的输入结果;输入参数分成必填和有默认值两种
- 长时间运行静态检测时后端偶尔会死掉
- 后端发回的消息增加一个id来区分多个前端窗口
- logBox允许复制/info栏允许复制


### 2023.3.12
- 设置页开发模式路径switch从默认false改成用utils中的is_dev()读取当前状态
- 7z打包前的计数没有筛选关键字,加上了;顺便修改了双层压缩的logger文本
...
- 调试用的快速启动py重新写了一下,原版放在desktop_v2整合文件夹里了,外面留下3个快捷入口:启动前端,启动后端,打包前端
